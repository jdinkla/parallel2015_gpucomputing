...
__kernel void mask(__global float* mask, __global float* dest, int width, int height)
{ 
...
__kernel void stencil(__global float* src, __global float* dest, int width, int height, const float value)
{
	const int x = get_global_id(0);
	const int y = get_global_id(1);
	if (x < width && y < height)
...
__kernel void zero(__global uchar4* dest, int width, int height)
{
	const int x = get_global_id(0);
	const int y = get_global_id(1);
	if (x < width && y < height)
